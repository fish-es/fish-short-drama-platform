...
} from '@/services/remote-media.service'
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    try {
      ...
      const clipId = uuid()
      db.run(
        "INSERT INTO video_clips (id, scene_id, video_id, status) VALUES (?, ?, ?, 'in_progress')",
        [clipId, sceneId, videoId],
      )
      saveDatabase()
      ...
      while (Date.now() - startTime < maxPollTime) {
        ...
        if (result.status === 'completed' && result.url) {
          const safeVideoUrl = await validateRemoteMediaUrl(result.url)
          db.run(
            "UPDATE video_clips SET status = 'completed', file_path = ? WHERE id = ?",
            [safeVideoUrl, clipId],
          )
          db.run(
            "UPDATE scenes SET state = 'VIDEO_READY', error_message = NULL WHERE id = ?",
            [sceneId],
          )
          saveDatabase()
          return NextResponse.json({ id: clipId, filePath: safeVideoUrl, videoId })
        }
